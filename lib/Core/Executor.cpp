		...
		if (f && f->isDeclaration()) {
				switch (f->getIntrinsicID()) {
						...
						case Intrinsic::maxnum: 
								{
										ref<Expr> left = eval(ki, 1, state).value;
										ref<Expr> right = eval(ki, 1, state).value;
										//TODO: Dumb emulation
										bindLocal(ki, state, left);
										break;
								}
						case Intrinsic::fabs: {
													  //TODO: Approximate floating point expression.
#if 0
...
//				if (CoreSearch == "fsum") {
				//No need for this, front ent pass wil take care
				//Create whitelist of things we allow

				if (!(f && f->getName()=="memcpy")) {
						//Enforce unintereprested functions
						auto callModel = 1; //0: Return symbolic, 1: return 0
						if (!ki->inst->getType()->isVoidTy()){
							if (callModel == 0) {
								Executor::ExactResolutionList rl;
								bindLocal(ki, state, ConstantExpr::create(0, (ki->inst->getType()->getPrimitiveSizeInBits())));
								ref<Expr> ret = eval(ki, 0, state).value;
								resolveExact(state, eval(ki, 0, state).value, rl, "make_symbolic");
								for (Executor::ExactResolutionList::iterator it = rl.begin(),
                                        ie = rl.end(); it != ie; ++it) {
										const MemoryObject *mo = it->first.first;
                                mo->setName(i->getName().str() + "_sym");

                                const ObjectState *old = it->first.second;
                                ExecutionState *s = it->second;
								if (old->readOnly) {
                                        terminateStateOnUserError(*s, "cannot make readonly object symbolic");
                                        return;
                                }

                                // FIXME: Type coercion should be done consistently somewhere.
                                bool res;
                                bool success __attribute__((unused)) = solver->mustBeTrue(
                                                s->constraints,
                                                EqExpr::create(
                                                        ZExtExpr::create(ret, Context::get().getPointerWidth()),
                                                        mo->getSizeExpr()),
                                                res, s->queryMetaData);
                                assert(success && "FIXME: Unhandled solver failure");
								if (res) {
										executeMakeSymbolic(state, mo, ki->inst->getName().str() + "_sym");
								} else {
										klee_warning("Reseroting to uninterpreted call model");
										bindLocal(ki, state, ConstantExpr::create(0, (ki->inst->getType()->getPrimitiveSizeInBits())));
								}

								}
							} else if (callModel == 1) {
								//TODO: Get const DataLayout& DL = M->getDataLayout(); and the nget type size
								// getPrimitiveSizeInBits gives target independent information
								bindLocal(ki, state, ConstantExpr::create(0, (ki->inst->getType()->getPrimitiveSizeInBits())));
							}
						}
						return;
...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki) {
		...
		//Analyze
#if 0
		std::cerr<< "Tracked Writes for state:"<<state.id<<"\n";
		 for (auto pair: state.writeTracker) {
                                                       auto mo = pair.first.first;
                                                       auto offset = pair.first.second;
...
#if 0
											   std::cerr<<"Global and local symbolics transforms"<<std::endl;
											   for (auto pair: state.symbolics) {
                                                                auto sym_mo = pair.first;
                                                                auto sym_os = state.addressSpace.findObject(&(*sym_mo));
                                                                //ref<Expr> offset = mo->getOffsetExpr(address);
                                                                ref<Expr> value = sym_os->read(0, Expr::Int8);
                                                                auto dest = pair.second->name;
                                                                std::cerr<<"\nDestination:";
                                                                std::cerr<<dest;
                                                                std::cerr<<"\nDestination Last Value:";
																for (int i =0;i < (sym_mo->size/8); i++) {
	                                                                sym_os->read(i, 8)->dump();
																}
                                                            }
#endif 
...
#if 01
#if 0
													   auto fake_l = ConstantExpr::alloc(0, value->getWidth());

                                                           auto fake = EqExpr::create(fake_l, value);
														   if (!fake->isFalse()) {
//                                                           		state.addConstraint(fake);
														   }
#endif 
													   std::string out_dir = output_directory;
                                                           std::error_code ec;
														   ExprSMTLIBPrinter printer;
                                                           llvm::raw_fd_ostream file(out_dir + "/global_"+ std::to_string(glob_file_idx++), ec, llvm::sys::fs::OF_Text);
											   ...
											   std::cerr<<"End of state:"<<state.id<<"\n";
											   ...
											   if (state.stack.size() <= 1) {
													   ...
													   for (auto pair: state.symbolics) {
                                                                ...
                                                                std::cerr<<dest;
                                                                std::cerr<<"\nDestination Last Value:";
                                                                sym_os->read(0, 8)->dump();
                                                       }
#endif

													   //Since we are doing single execution, we will most probably end up here:
													   std::string constraints;
													   if (result->getKind() != Expr::Constant) {
														   auto fake_l = ConstantExpr::alloc(0,ri->getReturnValue()->getType()->getPrimitiveSizeInBits());
														   auto fake = EqExpr::create(fake_l, result);
														   ExprSMTLIBPrinter printer;
														   std::string out_dir = output_directory;
														   std::error_code ec;
														   llvm::raw_fd_ostream file(out_dir + "/return_query_"+ std::to_string(file_idx++), ec, llvm::sys::fs::OF_Text);
														   printer.setOutput(file);
														   Query query(state.constraints, result);
														   printer.setQuery(query);
														   printer.generateOutput();
														   //state.addConstraint(fake);
...
#if 0
														   llvm::raw_fd_ostream file("./output.txt" + std::to_string(file_idx++), ec, llvm::sys::fs::OF_Text);
														   if (ec) {
        klee_warning("Error opening file");
    }
#if 0
														   for (auto pair: state.symbolics) {
	    			                                            auto sym_mo = pair.first;
													   ...
													   if (!isVoidReturn) {
															   ...
															   if (t != Type::getVoidTy(i->getContext())) {
																	   ...
																	   std::string constraints;
      																   getConstraintLog(state, constraints,Interpreter::SMTLIB2);
																	   klee_warning(constraints.c_str());
...
#if APPOX_FLOAT == 1
											   ref<Expr> value = eval(ki, 0, state).value;
											   ref<Expr> constantZero = ConstantExpr::alloc(0, value->getWidth());
                                               bindLocal(ki, state, SubExpr::create(constantZero, value));
#else
												ref<ConstantExpr> arg =
				...
				case Instruction::FDiv: {
												...
												ref<Expr> left = eval(ki, 0, state).value;
                                               ref<Expr> right = eval(ki, 1, state).value;
                                               bindLocal(ki, state, UDivExpr::create(left, right));
                                               break;
#else 
...
//#if APPOX_FLOAT == 1
												ref<Expr> left_s = eval(ki, 0, state).value;
                                                ref<Expr> right_s = eval(ki, 1, state).value;
                                                ref<Expr> result = SgeExpr::create(left_s, right_s);
												bool appx_float = 1;
//#else
												APFloat::cmpResult CmpRes;
												if (!appx_float) {
													ref<ConstantExpr> left = toConstant(state, eval(ki, 0, state).value,
																	"floating point");
													ref<ConstantExpr> right = toConstant(state, eval(ki, 1, state).value,
...
//#endif 
												...
												switch( fi->getPredicate() ) {
...
#if APPOX_FLOAT == 1
                                                                                        result = EqExpr::create(left_s, right_s);
#else
														...
														case FCmpInst::FCMP_UGT:
																#if APPOX_FLOAT == 1
                                                                result = SgtExpr::create(left_s, right_s);
#else
...
#if APPOX_FLOAT == 1
                                                                result = UgeExpr::create(left_s, right_s);
#else
																Result = (CmpRes != APFloat::cmpUnordered && (CmpRes == APFloat::cmpGreaterThan || CmpRes == APFloat::cmpEqual));
#endif
																break;
														...
														case FCmpInst::FCMP_OLT:
																#if APPOX_FLOAT == 1
                                                                result = SltExpr::create(left_s, right_s);
#else
														...
														case FCmpInst::FCMP_OLE:
																#if APPOX_FLOAT == 1
                                                                result = UleExpr::create(left_s, right_s);
#else
																Result = (CmpRes != APFloat::cmpUnordered && (CmpRes == APFloat::cmpLessThan || CmpRes == APFloat::cmpEqual));
#endif
																break;
														...
														case FCmpInst::FCMP_UNE:
																                                                                #if APPOX_FLOAT == 1
                                                                result = NeExpr::create(left_s, right_s);
#else
																Result = (CmpRes == APFloat::cmpUnordered || CmpRes != APFloat::cmpEqual);
#endif
																break;
...
void Executor::run(ExecutionState &initialState) {
		...
		//Let's get as many states and constraints as possible
//		while (!states.empty()) {
				...
				if (inBounds) {
						...
						if (isWrite) {
								if (os->readOnly) {
										...
										for (auto pair: state.symbolics) {
												auto sym_mo = pair.first;
												dest = pair.second->name;
												if (sym_mo == mo) {
													//Write update:
#if 0
													std::cerr<<"\nAddress:";
										            std::cerr<<mo->allocSite->getName().str();
													mo->getBaseExpr()->dump();
													std::cerr<<"\nAlloc site:";
													mo->allocSite->dump();
													std::cerr<<"\nDestination:";
													std::cerr<<dest;
//													std::cerr<<"\nDestination Last Value:";
//													os->read(offset, 8)->dump();

										            std::cerr<<"\nValue:";
										            value->dump();
#endif 

													std::pair<ref<const MemoryObject>, ref<Expr>> address = std::make_pair(sym_mo, offset);
//													auto fake_l = ConstantExpr::alloc(0, value->getWidth());
//													auto fake = EqExpr::create(fake_l, value);
													state.writeTracker[address] = value;
												}
										}
		...
		if (unbound) {
				if (incomplete) {
						...
						//Could be hardware
						klee_warning("Executing out of bounds: (Hoping it be MMIO)");
						//klee_warning(getAddressInfo(*unbound, address).c_str());
						if (!isWrite) {
								double v = 0;
								ref<Expr> result = ConstantExpr::fromMemory(&v, bytes * 8);
...
MemoryObject * Executor::allocate_recursive_memory(ExecutionState * state, Type * ty, Function * f, int depth) {
		...
		if (auto ptr = dyn_cast<llvm::PointerType>(ty)) {
				...
				uint64_t size = dataLayout.getTypeAllocSizeInBits(ptr->getElementType()) /8 ;
				//Allocate 400 bytes to handle edge case
				auto mo =
                                memory->allocate(size,
                                                /*isLocal=*/true, /*isGlobal=*/false,
                                                /*allocSite=*/first, /*alignment=*/8);

				ObjectState *ptr_os = bindObjectInState(*state, mo, false);

				if (auto * st = dyn_cast<StructType>(ptr->getElementType())) {
...
//                                          errs() << "Created GEP for field " << i << " of struct " << ST->getName() << ": " << *GEP << "\n";
										...
										offset += dataLayout.getTypeAllocSizeInBits(FieldType) / 8;
                                    }
				}
				...
				std::cerr<<"ArrayType"<<std::endl;
		} else if (auto ptr = dyn_cast<llvm::FunctionType>(ty)) {
				std::cerr<<"FunctionType"<<std::endl;
        }else if (auto int_ty = dyn_cast<llvm::IntegerType>(ty)) {
				std::cerr<<"IntegerType"<<std::endl;
				auto sz = int_ty->getBitWidth() / 8;
				auto mo =
                                memory->allocate(sz,
                                                /*isLocal=*/true, /*isGlobal=*/false,
                                                /*allocSite=*/first, /*alignment=*/8);
				auto name = "harness_sym_"+ std::to_string(harness_symbolics++);
				//executeMakeSymbolic(*s, mo, name);
				return mo;
        }else if (auto ptr = dyn_cast<llvm::StructType>(ty)) {
				std::cerr<<"StructType"<<std::endl;
        }else if (auto ptr = dyn_cast<llvm::VectorType>(ty)) {
				std::cerr<<"VectorType"<<std::endl;
        } else if (ty->isFloatingPointTy()) {
				std::cerr<<"Floating Type"<<std::endl;
//				fltSemantics semantics = ty->getFltSemantics();
//                auto sz = ty->getFltSemantics().getSizeInBits() / 8;
				unsigned sz = ty->getPrimitiveSizeInBits()/8;
                auto mo =
                                memory->allocate(sz,
                                                /*isLocal=*/true, /*isGlobal=*/false,
                                                /*allocSite=*/first, /*alignment=*/8);
...
void Executor::runFunctionAsMain(Function *f,
				...
				char **envp) {
		std::vector<ref<Expr> > arguments;
		std::vector<MemoryObject * > arguments_mo;
		...
		//Make symbolic here. if fsum
//		if (this->searcher->  == Searcher::CoreSearchType::FSUM) {
...
//		}
		...
		for (unsigned i = 0, e = f->arg_size(); i != e; ++i) {
				...
				bindObjectInState(*state, arguments_mo[i], true);
		}

		//Write the provided arguments to the symbolic/klee memory. We don't need this in UC, as we make them symbolic.
		if (argvMO) {
...
//				for (unsigned i = 0, e = f->arg_size(); i != e; ++i)
	                ...
	                //executeMakeSymbolic(*state, arguments_mo[i], "arg_" +std::to_string(i));
		}
